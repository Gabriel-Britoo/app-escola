...
export default function TelaAtivs({ route, navigation }) {
    ...
    async function salvarAtividade() {
        if (!titulo.trim() || !descricao.trim() || !turmaSelecionada) {
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.selectContainer}>
                <Text style={styles.selectLabel}>Selecione a turma:</Text>
                    ...
                    {turmas && turmas.length > 0 ? (
                        ...
                        >
                            <Text style={styles.pickerText}>
                                {turmas.find(t => t.id === turmaSelecionada)?.nome_turma || 'Selecione uma turma'}
                            </Text>
                            <Ionicons name="chevron-down" size={24} color="#A31D1D" />
                        </TouchableOpacity>
                    ) : (
                        <Text style={styles.noTurmasText}>Nenhuma turma disponível</Text>
                    )}
                </View>
            </View>
            ...
            <Modal transparent visible={modalTurmaVisible} animationType="fade">
                <TouchableOpacity
                    style={styles.overlay}
                    activeOpacity={1}
                    onPress={() => setModalTurmaVisible(false)}
                >
                    <View style={[styles.modalContainer, styles.turmaModalContainer]}>
                        <Text style={styles.tituloModal}>Selecione a Turma</Text>
                        <ScrollView style={styles.turmaList}>
                            {turmas.map((turma) => (
                                <TouchableOpacity
                                    key={turma.id}
                                    style={[
                                        styles.turmaListItem,
                                        turmaSelecionada === turma.id && styles.turmaListItemSelected
                                    ]}
                                    onPress={() => {
                                ...
                                >
                                    <Text style={[
                                        styles.turmaListText,
                                        turmaSelecionada === turma.id && styles.turmaListTextSelected
                                    ]}>
                                        {turma.nome_turma}
                                    </Text>
...
const styles = StyleSheet.create({
    ...
    turmaListItem: {
        ...
        padding: 15,
        borderBottomWidth: 1,
        borderBottomColor: "#ECDCBF",
        backgroundColor: "#fff",
    },
    turmaListItemSelected: {
        backgroundColor: "#A31D1D",
    },
    turmaListText: {
        fontSize: 16,
